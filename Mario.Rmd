...
top10g %>%
  ggplot(aes(genres, popularity)) + geom_col(fill = 'royalblue3') + theme(axis.text.x = element_text(size = 12, angle = 45, vjust = 1, hjust = 1)) + coord_cartesian(ylim = c(70, 82))
```
...
top10g %>%
  ...
  ggplot(aes(name, value)) + geom_col(position = 'dodge') + facet_wrap(~genres) + coord_flip()
```